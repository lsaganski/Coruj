...
class FormsViewController: BaseViewController, MenuDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    func loadData() {
        ...
        let url = "\(Constants.Api.MAIN_PATH)\(Constants.Api.FORMS_PATH)"
        ...
        Alamofire.request(url, method: HTTPMethod.post, parameters: parameters, encoding: JSONEncoding.default, headers: nil)
            .responseArray(completionHandler: { (response: DataResponse<[Formulario]>) in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let obj: Formulario = self.list.filter {
            $0.date == header
            }[0].data[indexPath.row] as! Formulario
        ...
        novo.image = #imageLiteral(resourceName: "crj_ic_novo_forms")
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        vc.link = obj.EnqueteForms
        vc.mytitle = "Formulários"
