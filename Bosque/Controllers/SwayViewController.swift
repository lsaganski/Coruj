...
class SwayViewController: BaseViewController, MenuDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    func loadData() {
        ...
        let url = "\(Constants.Api.MAIN_PATH)\(Constants.Api.SWAY_PATH)"
        ...
        Alamofire.request(url, method: HTTPMethod.post, parameters: parameters, encoding: JSONEncoding.default, headers: nil)
            .responseArray(completionHandler: { (response: DataResponse<[Sway]>) in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let obj: Sway = self.list.filter {
            $0.date == header
            }[0].data[indexPath.row] as! Sway
        ...
        novo.image = #imageLiteral(resourceName: "crj_ic_novo_sway")
